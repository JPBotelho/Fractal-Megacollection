Shader "Fractals/Coloring Techniques/Smooth"
{
	Properties
	{
	...
	SubShader
	{
		...
		Pass
		{
			...
			fixed4 frag (v2f i) : SV_Target
			{
				...
				int iteration = 0;
				while (IsBounded(z, 400) && iteration < _Iter)
				{	
					z = cmul(z, z);
					z += c;					
					iteration++;
				...
				float linearColoring = LinearColoring(z, iteration, 2);

				return linearColoring * _Background / _Dividend;
			}		    
			ENDCG
		}
	}
