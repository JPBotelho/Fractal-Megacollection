Shader "Fractals/Newton/Converge-Time"
{
	Properties
	{
		...
		_Zoom ("Zoom", Range (0.0001, 1000)) = 1

		_Julia ("Julia Offset", Vector) = (0, 0, 0, 0)
		_Nova ("Nova", Vector) = (0, 0, 0, 0)

		_Tolerance ("Tolerance", Range (0, 1)) = 0.0001
		_Position ("Offset", Vector) = (0, 0, 0, 0)
		_Dividend ("Dividend", Range (1, 250)) = 1
	...
	SubShader
	{
		...
		Pass
		{
			...
			fixed4 frag (v2f i) : SV_Target
			{
				...
				{
					z -=  cmul(_Nova.xy, cdiv(Function(z), Derivatice(z))) + _Julia.xy;

					for (int i = 0; i < 3; i++)
					{
						if (hasConverged (z, roots[i], _Tolerance))
						{							
							return colors[i] * iteration / _Dividend;
						}						
					}
					iteration++;
				}
